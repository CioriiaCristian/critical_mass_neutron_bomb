...
def integrand(p, x_bar):
    ...
    Args:
        x_bar(float) : This parameter gives the neutron distribution at a location in the admitted domain <- (0, np.pi)
    ...
    initial_distribution = core.choose_distribution(
        dimension="1d", distribution_type="gaussian", symmetry=""
    )
...
def neutron_distribution(ap, x, t):
    ...
    for i in list(range(1, Config.coefficient_size_1d)):
        _sum += (
            ap[i]
            * np.exp((Config.eta - Config.mu * (i * np.pi / L) ** 2) * t)
            * np.sin(i * np.pi * x / L)
        )
    return _sum
...
def main():
    ...
    ap = np.zeros(
        Config.coefficient_size_1d
    )  # the list containing the fourier coefficients
    for i in list(range(1, Config.coefficient_size_1d)):
