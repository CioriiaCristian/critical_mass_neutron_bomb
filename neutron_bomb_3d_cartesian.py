...
y_bar_domain = y_domain*(np.pi/L) # The conversion is needed for the simpson algorithm
z_domain = np.linspace(0,L,N)
z_bar_domain = y_domain*(np.pi/L) # The conversion is needed for the simpson algorithm
time_domain = np.linspace(0,Config.time_limit_3d_cartesian,N)


def integrand(p,q,r, x_bar, y_bar, z_bar):
    
    initial_distribution = core.choose_distribution(dimension='3d', symmetry='cartesian',distribution_type='special')

    return ((2/np.pi)**3)*initial_distribution((L/np.pi)*x_bar,(L/np.pi)*y_bar,(L/np.pi)*z_bar)*np.sin(p*x_bar)*np.sin(q*y_bar)*np.sin(r*z_bar)
...
def neutron_distribution(apqr, x, y,z, t):
    ...
    for i in range(1,n):
        for j in range(1,n):
            for k in range(1,n):
                _sum+= apqr[i,j,k]*np.exp(
                    Config.eta*t - Config.mu*((i*np.pi/L)**2 + (j*np.pi/L)**2 + (k*np.pi/L)**2)*t)*np.sin(i*np.pi*x/L)*np.sin(j*np.pi*y/L)*np.sin(i*np.pi*z/L)
    return _sum
            ...
            for k in list(range(1,n)):
                ...
                for x_bar in x_bar_domain:
                    ...
                    for y_bar in y_bar_domain:
                        # Fix the y
                        _integrated_z_slice.append(simpson(integrand(i,j,k,x_bar,y_bar,z_bar_domain),z_bar_domain))
                    
                    # Integration over y axis
                    _integrated_y_strips.append(
                        simpson(
                            np.array(_integrated_z_slice),y_bar_domain
                        )
                    )
                # Integration over x
        ...
        for x_idx, x_loc in enumerate(tqdm(x_domain)):
            for y_idx,y_loc in enumerate(y_domain):
                    for z_idx,z_loc in enumerate(z_domain):
                        values[x_idx,y_idx,z_idx,t_idx] = neutron_distribution(apqr,x_loc,y_loc,z_loc,t)
