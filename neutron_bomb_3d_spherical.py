...
def integrand(p,r_bar):
    ...
    initial_distribution = core.choose_distribution(dimension='3d', symmetry='spherical',distribution_type='special')

    return (2*r1/(np.pi**2))*r_bar*(initial_distribution(r_bar*(r1/np.pi)))*np.sin(p*r_bar)
...
def neutron_distribution(ap,r, t):
    ...
    if r < 1e-8:
        ...
        for i in list(range(1,n)):
            _sum+= ap[i-1]*(1/r)*np.exp((Config.eta - Config.mu *( i * np.pi / r1 )**2)*t)*np.sin(i*np.pi*r/r1)
        return _sum
