...
def integrand(p,q, x_bar, y_bar):
    ...
    initial_distribution = core.choose_distribution(dimension='2d', symmetry='cartesian',distribution_type='special')

    return (4/(np.pi**2))*initial_distribution((L/np.pi)*x_bar,(L/np.pi)*y_bar)*np.sin(p*x_bar)*np.sin(q*y_bar)

def neutron_distribution(apq, x, y, t):
    _sum = 0 
    for i in range(0,Config.coefficient_size_2d):
        for j in range(Config.coefficient_size_2d):
            _sum+= apq[i,j]*np.exp(Config.eta*t - Config.mu*((i*np.pi/L)**2 + (j*np.pi/L)**2)*t)*np.sin(i*np.pi*x/L)*np.sin(j*np.pi*y/L)
    return _sum
    ...
    for i in list(range(n)):
        for j in list(range(n)):
            ...
            for x_bar in x_bar_domain:
                _integrated_y_strips.append(simpson(integrand(i,j,x_bar, y_bar_domain),y_bar_domain))
